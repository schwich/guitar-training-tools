 ...
 */
export const standardGuitarTuningSPN = ["E4", "B3", "G3", "D3", "A2", "E2"];

export const standardGuitarTuning = ["E", "B", "G", "D", "A", "E"];
...
export function generateNoteSequenceFromFingering(
  ...
  for (let i = 0; i < length; i++) {
    // push #(length) of notes to noteSequence
...
export function createChromaticSequenceSPN(
  startNote: string,
  startOctaveNum: number,
  length: number
): INoteSPN[] {
 ...
 * @param length
 */
export function createChromaticSequence(
  startNote: string,
  length: number
): INote[] {
  let nextIdx = getChromaticArrIdx(startNote);
 ...
 * @param pattern
 */
export function createNoteSequenceFromIntervalPattern(
  tonic: string,
  pattern: string
) {
