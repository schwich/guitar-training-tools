...
export default class GameDisplay extends React.Component<Props, object> {
  render() {
    ...
    return (
      <Paper
        elevation={1}
        style={{ minWidth: 200, maxWidth: 400, marginLeft: 10, padding: 15 }}
      >
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          <Button
            style={{ marginTop: 10 }}
            variant="contained"
            onClick={() => this.props.handleClearScore()}
          >
            Clear Score
          </Button>
          {isGameRunning ? (
...
function GamePercentageCorrect({
  numCorrect,
  totalGuesses,
}: GamePercentageProps) {
