...
export default class FretboardControlPanel extends React.Component<
  Props,
  State
> {
  private numFretsMenuOptions = ["12", "15", "22", "24"];
  ...
  handleStringSelect = (event: React.ChangeEvent<HTMLSelectElement>) => {
    if (event.target && event.target.value) {
      ...
      for (let num of event.target.value) {
        // typescript doesn't like this object, it's an array but it seems to thing it's a string
      ...
      <Paper elevation={1} style={{ maxWidth: 300, marginLeft: 10 }}>
        ...
        >
          <div id="gameControlPanelNumFretsControl">
            <FormControl>
              <InputLabel htmlFor="num-frets-select">
                Number of Frets
              </InputLabel>
              <Select
          ...
          <div id="gameControlPanelStringsToGuessControl">
            <FormControl style={{ minWidth: 200, maxWidth: 300 }}>
              <InputLabel htmlFor="multiple-string-checkbox">
                Strings to Guess for
              </InputLabel>
              <Select
                multiple
              ...
              >
                {this.guitarStrings.map((_, idx) => (
                  <MenuItem key={idx} value={idx}>
                    ...
                    />
                    <ListItemText
                      primary={`${this.guitarStrings[idx].number} - ${this.guitarStrings[idx].name}`}
                    />
                  </MenuItem>
                ))}
              </Select>
